...
/*global jQuery */
//function on_gridsearchfeedback(data) { $(_connection).trigger("on_gridsearchfeedback", [jQuery.parseJSON(data)]); return false; };
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
                ...
                case "tagatweb":
                    ...
                    $(_connection).unbind("on_gridsearchfeedback");
                    $(_connection).bind("on_gridsearchfeedback", function (event, data,_iAprox,_page) {
                        //self.on_dataSearchFeedbackCommon(data);
                        if(data == undefined || data == null)
                        return;
                        
                        
                        //alert("on_gridsearchfeedback" + JSON.stringify(data));
                          $('#_MHSearchAreaSearchTextBox').val(JSON.stringify(data));                      
                            var girddataobj = {};
                            girddataobj.page = ts.p.page = _page;
                            girddataobj.total = _iAprox == 0 ? 1 :  ( _iAprox / ts.p.rowNum ) + 1 ;
                            girddataobj.records = _iAprox;
                            girddataobj.rows = [];

                           
                            for(i=0; i < data.length; i++)
                            {
                                 var  _resultrw = data[i];
                                 var  _gridresultrow = {};
                                 _gridresultrow.cell = [];

                                 for(var ii=1; ii < ts.p.colModel.length; ii++)
                                 {
                                    for(var iii=0; iii < _resultrw.length; iii++)
                                    {
                                       if (_resultrw[iii].cStrID == ts.p.colModel[ii].name)
                                       {
                                            _gridresultrow.cell.push(_resultrw[iii].cStrValue);

                                            if (ts.p.colModel[ii].name == "docid")
                                            _gridresultrow.id = _resultrw[iii].cStrValue;
                                       
                                       }
                                    }
                                 }
                                 girddataobj.rows.push(_gridresultrow);
                           ...
                           //alert("addJSONData" + ts.p.colModel);
                           addJSONData(girddataobj,ts.grid.bDiv,rcnt,npage>1,adjust); }
