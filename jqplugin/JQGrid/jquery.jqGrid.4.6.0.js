...
(function ($) {
                ...
                if (!ts.grid.hDiv.loading) {
                    ...
                    switch (dt) {
                        ...
                        case "script":
                            $.ajax($.extend({
                                ...
                                success: function (data, st, xhr) {
                                    if (data != undefined && data.bSUCCESS) {
                                        var JqJsonObject = new Object();
                                        JqJsonObject.records = data.nTOTAL;//total row count                                        
                                        JqJsonObject.rows = data.oRESULT;//result data array
                                        //data.nCOUNT current rocord count
                                        //JqJsonObject.page = 1;//current page 
                                        JqJsonObject.total = Math.ceil(data.nTOTAL / ts.p.rowNum);//total pages count                                           
                                        var jsonString = JSON.stringify(JqJsonObject);
                                        var data = JSON.parse(jsonString);
                                    }
                                    else {
                                        var JqJsonObject = new Object();
                                        JqJsonObject.records = 0;//total row count                                        
                                        JqJsonObject.rows = [];//result data array                                        
                                        JqJsonObject.total = 0;//total pages count                                           
                                        var jsonString = JSON.stringify(JqJsonObject);
                                        var data = JSON.parse(jsonString);
                                    }
                        ...
                        case "clientside":
                            ...
                            break;
                        case "postback":
                            objParam = ts.p.tagret.funcGetRequestParams(ts.p.postData);
                            $.ajax({
                                url: __GAPPENGINE_DOMAIN + "Postback",
                                dataType: "json",
                                cache: false,
                                data: objParam,
                                type: 'POST',
                                success: function (data, textStatus, xOptions) {
                                    data = ts.p.tagret.funcPopulateResult(data);
                                    if (data != undefined) {
                                        data.total = Math.ceil(data.records / ts.p.rowNum);//total pages count                                           
                ...
                if (ts.p.multiSort) {
                    ...
                    if (!ts.p.viewsortcols[0]) {
                        if (ts.p.lastsort !== idxcol) {
                            ...
                            $("span.ui-grid-ico-sort", newSelectedTh).show();
                        }
                    }
    ...
    $.jgrid.extend({
        ...
        updatePaginator: function (records, totalPages) {
            return this.each(function () {
                var $t = this;
                $t.p.records = records;
                $t.p.lastpage = totalPages;
                $t.updatepager(true, false);
            });
        }
    });
